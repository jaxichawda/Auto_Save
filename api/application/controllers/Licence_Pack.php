...
class Licence_Pack extends CI_Controller 
{	
	...
	public function addTempData()
	{
		...
		if ($post_Licence_Pack) 
		{
			...
			{
				$result = $this->Licence_Pack_model->editTempData($post_Licence_Pack);
				if($result)
				{
					echo json_encode('');	
				}	
			...
			{
				$result = $this->Licence_Pack_model->addTempData($post_Licence_Pack);
				if($result)
				{
	...
	{			
		...
		{
			$data=[];
			$data=$this->Licence_Pack_model->getTempById($Licence_Pack_id);
			echo json_encode($data);
		}
	}
	...
	{		
		$data=$this->Licence_Pack_model->getTempData();		
		echo json_encode($data);
	}
